...
serve(async (req) => {
  ...
  try {
    ...
    // Try to get authenticated user (optional for one-time)
    ...
    const origin = req.headers.get("origin") || "https://winthenightproductions.lovable.app";

    let lineItems;
    if (customAmount) {
      // Custom one-time amount (in cents)
      lineItems = [{
        price_data: {
          currency: "usd",
          product: "prod_U9NyZHohFQkW13", // One-Time Donation product
